...
int main(void)
{
	...
	while (!glfwWindowShouldClose(gctx.window))
	{
						 ...
						 NK_WINDOW_MINIMIZABLE | NK_WINDOW_TITLE))
		{
			...
			if (nk_option_label(ctx, "Crop image", !projection))
				projection = false;
			if (nk_option_label(ctx, "Project image", projection))
				projection = true;
			...
			if (nk_tree_push(ctx, NK_TREE_TAB, "Image Channel 1", NK_MAXIMIZED))
			{
				...
				if (nk_button_label(ctx, ""))
				{
					if (load_channel_image(&ch1))
					{
						...
						glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 5760, 2880, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
						glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
						glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		...
		if (ch1.enabled && projection && !ch1.video)
		{
			if (runonce)
			{
				...
				for (int i = 8017; i <= 8991; i++)
				{
					/* code */
					sprintf(filein, "C:\\DMGMORI_Development\\msys64\\home\\artsimow\\build\\frostorama\\seq\\out%04d.png", i);
					imgdata = stbi_load(filein, &ch1.width, &ch1.height, &ch1.color_depth, 0);
					glDeleteTextures(1, &ch1.tex);
					glGenTextures(1, &ch1.tex);
					/* glActiveTexture(GL_TEXTURE0); */
					glBindTexture(GL_TEXTURE_2D, ch1.tex);
					glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
									GL_CLAMP_TO_EDGE);
					...
					glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
									GL_LINEAR);
					glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, ch1.width,
								 ch1.height, 0, GL_RGB, GL_UNSIGNED_BYTE,
								 imgdata);
					glViewport(0, 0, 5760, 2880);
					glClear(GL_COLOR_BUFFER_BIT);
					vec4 crop;
					...
					glReadPixels(0, 0, 5760, 2880, GL_RGB, GL_UNSIGNED_BYTE, buf);

					/* Save OpenGL framebuffer to a PNG file with stb_image_write */
					stbi_write_png_compression_level = 0;
					/* code */
					stbi_flip_vertically_on_write(true);
					sprintf(fileout, "G:\\out\\%04d.bmp", i);
					stbi_write_bmp(fileout, 5760, 2880, 3, buf);
					stbi_image_free(imgdata);
				}
			}
			runonce = true;
